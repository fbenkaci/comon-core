...
/*   Updated: 2025/03/01 12:06:20 by fbenkaci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_img2(t_data *img)
{
	...
	img->Exit = mlx_xpm_file_to_image(img->mlx, "exit.xpm", &img->img_height,
		&img->img_width);
	if (!img->Exit)
	{
		mlx_destroy_image(img->mlx, img->wall);
...
int	init_img(t_data *img)
{
	...
	if (!img->wall)
		return (0);
	img->Item = mlx_xpm_file_to_image(img->mlx, "item.xpm", &img->img_height,
			&img->img_width);
	if (!img->Item)
	{
		mlx_destroy_image(img->mlx, img->wall);
	...
	while (img->grid[i] != NULL)
	{
		...
		while (img->grid[i][j])
		{
			if (img->grid[i][j] == '1')
				...
				mlx_put_image_to_window(img->mlx, img->win, img->Exit, i
					* width, j * height);
			else if (img->grid[i][j] == 'C')
				mlx_put_image_to_window(img->mlx, img->win, img->Item, i
					* width, j * height);
			j++;
		}
		i++;
